...
using DepList = std::vector<std::pair<std::string, std::vector<std::string>>>;
...
public:
  ...
  Parser* Build();
  Parser * BuildForDependency(MainTypeTable<TypeTableValData>* pMainTable);
  TreeNode* DisplayAbstractSyntaxTree();
  MainTypeTable<TypeTableValData>* GetTypeTable();

  std::pair<std::string, std::vector<std::string>>* getDepPair();
...
private:
  ...
  RuleForDependencyCheck* pRuleForDependencyCheck;
